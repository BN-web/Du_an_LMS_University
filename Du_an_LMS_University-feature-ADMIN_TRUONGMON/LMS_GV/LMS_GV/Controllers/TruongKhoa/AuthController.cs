...
namespace LMS_GV.Controllers.TruongKhoa
{
    ...
    public class TruongKhoaAuthController : ControllerBase
    {
        ...
        private readonly ILogger<TruongKhoaAuthController> _logger;

        public TruongKhoaAuthController(AppDbContext db, IConfiguration config, ILogger<TruongKhoaAuthController> logger)
        {
            _db = db;
            _config = config;
            _logger = logger;
        ...
        public async Task<IActionResult> Login([FromBody] TruongKhoaLoginRequest req)
        {
            ...
            if (!(roleId == 3 ||
                  roleName.Equals("Trưởng khoa", StringComparison.OrdinalIgnoreCase)))
            {
                return Unauthorized(new { message = "Không có quyền Trưởng khoa" });
            }
            ...
            var claims = new List<Claim>
            {
                ...
                new Claim(ClaimTypes.Role, roleName)  // "Trưởng khoa"
