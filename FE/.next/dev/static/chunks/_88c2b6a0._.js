...
function LichDayPage() {
        ...
        "LichDayPage.useEffect": ()=>{
            const fetchData = {
                "LichDayPage.useEffect.fetchData": async ()=>{
                    try {
                        setIsLoading(true);
                        const [giangVienRes, lopRes, phongRes] = await Promise.all([
                        ...
                        const mappedLecturers = Array.isArray(giangVienRes) && giangVienRes.length > 0 ? giangVienRes.map({
                            "LichDayPage.useEffect.fetchData": (item)=>({
                                    id: item.id,
                                    ten: item.label,
                                    trangThai: "Hoạt động"
                                })
                        ...
                        setLopOptions(Array.isArray(lopRes) ? lopRes : []);
                        setPhongOptions(Array.isArray(phongRes) ? phongRes : []);
                        // Hiện tại chưa có API lịch dạy/thi cho Trưởng khoa, nên để trống danh sách
                        setLichDayData([]);
                    } catch (error) {
    ...
    // Lấy danh sách lớp học và phòng học từ dữ liệu lịch (nếu sau này có)
    ...
    const handleCreateSchedule = async ()=>{
        ...
        try {
            ...
            // Sau này có API lịch dạy thì sẽ reload lại danh sách
...
_s(LichDayPage, "+Qvqtvz7P+lBtqpjrCpbj3zLSU4=", false, function() {
    return [
...
//# sourceMappingURL=_88c2b6a0._.js.map