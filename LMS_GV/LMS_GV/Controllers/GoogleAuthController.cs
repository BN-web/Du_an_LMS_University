...
namespace LMS_GV.Controllers
{
    ...
    public class GoogleAuthController : ControllerBase
    {
        ...
        /// <summary>
        /// Đăng nhập / đăng ký cho Trưởng khoa bằng Google
        /// </summary>
        ...
        public async Task<ActionResult<GoogleLoginResponseDTO>> LoginWithGoogleTruongKhoa([FromBody] GoogleLoginRequestDTO request)
        {
            try
            {
                ...
                string roleName = "Trưởng khoa";

                if (nguoiDung == null)
                {
                    // 2.1. Tạo NguoiDung mới với vai trò Trưởng khoa
                    var tenDangNhap = request.Email.Split('@')[0];
                    var tenDangNhapGoc = tenDangNhap;
                    int counter = 1;
                    ...
                    // Tạo hồ sơ Giảng viên với chức vụ Trưởng khoa
                    // Cần có KhoaId - có thể để null hoặc yêu cầu từ request
                    var giangVien = new GiangVien
                    {
                        NguoiDungId = nguoiDung.NguoiDungId,
                        ChucVu = "Trưởng khoa",
                        KhoaId = null, // Có thể cập nhật sau
                        CreatedAt = DateTime.Now
                    };
                ...
                else
                {
                    ...
                    // Kiểm tra xem có hồ sơ Giảng viên chưa
                    var giangVien = await _context.GiangViens
                        .FirstOrDefaultAsync(gv => gv.NguoiDungId == nguoiDung.NguoiDungId);
                ...
                // 4. Sinh JWT token
                var token = GenerateJwtToken(nguoiDung, vaiTroId.ToString());
